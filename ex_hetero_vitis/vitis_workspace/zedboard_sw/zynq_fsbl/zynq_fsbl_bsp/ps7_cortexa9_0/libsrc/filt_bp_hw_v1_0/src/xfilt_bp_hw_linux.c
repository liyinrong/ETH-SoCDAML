...
typedef struct {
    ...
    XFilt_bp_hw_uio_map maps[ MAX_UIO_MAPS ];
} XFilt_bp_hw_uio_info;

/***************** Variable Definitions **************************************/
static XFilt_bp_hw_uio_info uio_info;
...
static int uio_info_read_name(XFilt_bp_hw_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFilt_bp_hw_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFilt_bp_hw_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFilt_bp_hw_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFilt_bp_hw_Initialize(XFilt_bp_hw *InstancePtr, const char* InstanceName) {
	XFilt_bp_hw_uio_info *InfoPtr = &uio_info;
