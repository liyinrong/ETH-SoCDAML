...
#define HDR_LENGTH   (~(1<<(bitsizeof(int)-1)))
#define HDR_FREE     (~(1<<(bitsizeof(int)-1)))
#define HDR_RESERVED (1<<(bitsizeof(int)-1))
...
const int nr_of_blocks = 64;//heapsize 1K  //16*64=1K
//const int nr_of_blocks =  1024;// referes to heap of 16K ( 16*1024=16K)
#ifndef __clang__
...
INLINE void set_head(MBlock_* b, int res, int len) {
    b->head = res << (bitsizeof(int)-1) | (len & HDR_LENGTH);
}
