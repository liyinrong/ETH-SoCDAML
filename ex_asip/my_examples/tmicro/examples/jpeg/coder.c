...
#define CONST_BITS  8
#define FIX_0_382683433  ((/*int*/long)   98)		/* FIX(0.382683433) */
#define FIX_0_541196100  ((/*int*/long)  139)		/* FIX(0.541196100) */
#define FIX_0_707106781  ((/*int*/long)  181)		/* FIX(0.707106781) */
#define FIX_1_306562965  ((/*int*/long)  334)		/* FIX(1.306562965) */
...
INLINE int calc_nrbits(int in)
{
  ...
  int val = in<0?-in:in;
  // FIXME: 16bit // if (val>=(1<<16)) { val>>=6; out+=6; }
...
{
  /*int*/ SWORD tmp0, tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7;
  /*int*/ SWORD tmp10, tmp11, tmp12, tmp13;
  /*int*/ SWORD z1, z2, z3, z4, z5, z11, z13;
...
INLINE /*int*/ SWORD quantization(DCTELEM DU_DCT[64],
	         /*int*/ SWORD     quanttbl[64],
		 SWORD   DU_ZZ[64])
{
  ...
  for (int i = 0; i < 64; i++) chess_unroll_loop(2) {
      ...
      /*int*/ SWORD itemp = (DU_DCT[i]*(long)quanttbl[i])>>15;
      if (DU_DCT[i]<0) itemp++;
...
/*int*/SWORD*       blk_quant[6] = { quantY, quantY, quantY, quantY, quantC,  quantC  };
  ...
  if (blk<4) {
	...
	for (int x2=0;x2<4;x2++) {
		...
		for (int x=0;x<2;x++) chess_unroll_loop(*) {
		    ...
		    DU[(y2*2+y)*8+(x2*2+x)]  = (((BYTE)( ( 19595L*R + 38470L*G +  7471L*B)>>16 ))&0xFF) - 128;
		    //Cb += (int)((SBYTE)( (-11058*R - 21708*G + 32768*B)>>16 ) );
		    Cb += (int)((SBYTE) ((32768L*B - 11058L*R - 21708L*G) >>8)) >>8;
		    //Cr += (int)((SBYTE)( ( 32768*R - 27438*G -  5328*B)>>16 ) );
		    Cr += (int)((SBYTE) ((32768L*R - 27438L*G -  5328L*B) >>8)) >>8;
		  }
	      }
 ...
 vlc_init:
  {
	...
	loading:
	  ...
	  // for debugging
	  //for (int i=0; i < 64; i++) chess_report(DU[i]);
	}
      }
    }
