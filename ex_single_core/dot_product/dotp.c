...
unsigned int dotproduct_loopunroll(unsigned int acc, unsigned char* vA, unsigned char* vB, unsigned int N)
{
    ...
    asm volatile(
            ...
            "c.nop;"
        #endif
            "lp.setup x0,%[n_elem],stop_lloop;"
            "p.lbu %[a], 1(%[addrA]!);"
            "p.lbu %[b], 1(%[addrB]!);"
            "p.lbu %[d], 1(%[addrA]!);"
            "p.lbu %[e], 1(%[addrB]!);"
            "p.mac %[c], %[a], %[b];"
            "stop_lloop: p.mac %[c], %[d], %[e];"
            : [c] "+r" (acc), [a] "+r" (elemA), [b] "+r" (elemB), [d] "+r" (elemD), [e] "+r" (elemE), [addrA] "+r" (vA), [addrB] "+r" (vB) : [n_elem] "r" (N/2));
...
unsigned int dotproduct_loopunroll_simd(unsigned int acc, unsigned char* vA, unsigned char* vB, unsigned int N)
{
    ...
    asm volatile(
            ...
            "c.nop;"
        #endif
            "lp.setup x0,%[n_elem],stop_llloop;"
            "p.lw %[a], 4(%[addrA]!);"
            "p.lw %[b], 4(%[addrB]!);"
            "p.lw %[d], 4(%[addrA]!);"
            "p.lw %[e], 4(%[addrB]!);"
            "pv.sdotup.b %[c], %[a], %[b];"
            "stop_llloop: pv.sdotup.b %[c], %[d], %[e];"
            : [c] "+r" (acc), [a] "+r" (elemA), [b] "+r" (elemB), [d] "+r" (elemD), [e] "+r" (elemE), [addrA] "+r" (vA), [addrB] "+r" (vB) : [n_elem] "r" (N/8)
    );
